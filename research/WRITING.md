...
3. **Slop-word greplist** — before any draft is shown or submitted, grep and
   ...
   2025–26 additions (Cherryleaf Feb 2026, Reuters Institute Dec 2025,
   Wikipedia signs-of-AI-writing July 2026 revision):
   `emphasiz|enhanc|highlight|it.s worth noting|navigate|robust|holistic|nuanced|multifaceted|seamless|resonate|commendable|quietly|fundamentally|remarkably`
   Note: single words are now the WEAK signal — the 2023-era words (delve,
   tapestry, intricate, meticulous) dropped off sharply in 2025; don't
   over-correct against ghosts. Structure is the strong signal.
3b. **Structural tells, ranked (2025–26 sources — Bloomberry Apr 2026,
   tropes.fyi, Wikipedia July 2026, Forbes "Seven Tells" Jun 2025):**
   1. Negative parallelism: "It's not X, it's Y" / "not just X but Y" —
      the #1 current tell. State the positive claim, no negated strawman.
   2. Uniform sentence/paragraph shape ("relentless balance") — vary length
      by meaning; let sections be asymmetric.
   3. Rule-of-three lists — cut to the one or two items actually meant
      (a genuine three-item enumeration of real things is fine).
   4. Resolution/summary closers — end paragraphs on the last new fact,
      not a recap; no "In sum, these findings..." sentences.
   5. Bold-phrase-then-colon list items ("**Scalability**: ...") — prose
      or plain bullets instead.
   6. Fake-question pivots ("The result? ...") and rhetorical-question
      transitions — present the answer directly.
   7. Copula avoidance ("serves as / stands as / represents") — write "is".
   8. Colon-appositive density ("X: a Y, a Z") — weak alone, signals in
      combination; more than ~1 per abstract-length passage is a smell.
   9. Hedged-confident tone ("arguably", "in many ways") — hedge only where
      evidence is thin, state strong results plainly.
   10. Unnamed attribution ("studies have shown") — name it or cut it.
   Anti-laundering rule (Wikipedia's own warning): fix the underlying
   vagueness/padding, not just the surface marker — cosmetic scrubbing
   makes prose worse and detection harder, and over-corrected sterile prose
   is itself becoming a tell.
4. **Em dashes:** the tell is DENSITY and monotony, not presence (2–3
   natural uses fine; one for every aside is machine-like). Where one does
   connector-phrase work ("— and this is critical —"), restructure the
   sentence instead of just swapping punctuation. Don't strip every dash —
   over-correction hollows the prose.
