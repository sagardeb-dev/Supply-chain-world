"""Tier 2 — substrate: the agent's books (shipments, inventory, contracts as
data). Deterministic given the weekly hidden states; no randomness, no hidden
state of its own.

Transit-week causality: a chokepoint affects the ships that are AT it that
week, never the ships merely ordered that week. A Suez ship meets the canal at
dispatch+suez_chokepoint_offset; the canal's state THAT week decides through /
backlog / queue. A queued ship waits one week, then proceeds if clear or
diverts around the Cape. A Cape ship meets its congestion point at
dispatch+cape_chokepoint_offset."""
