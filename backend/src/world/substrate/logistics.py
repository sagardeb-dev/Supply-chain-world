"""Tier 2 — substrate: voyage resolution. Dispatch this week's order, move
every in-flight ship one week, land arrivals, consume demand, total the cost.
Deterministic given the weekly hidden states.

The kappa cost-coupling (a spot shortfall during a brewing disruption) is NOT
computed here -- it is the one tier-3 term, in couplings.crisis_backorder.
...
def resolve_week(books: Books, qty: int, supplier: str | None,
    ...
    # The Becker JV coupling (A8.2): reads BOTH factors but lives in the reward,
    # so the belief stays factored. The two-factor read is in couplings.py.
