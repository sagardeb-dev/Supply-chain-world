...
agent orders stock (a free `qty` from 0 up to `order_max`), picks a supplier and
a shipping route, and manages supplier contracts, while several **hidden latent
factors** evolve underneath and leak only noisy or indirect signals. The world
is the simulator the agents and the classical-policy baselines both run against.

A `World` is parameterized by `(config, registry)` — *a different registry is a
different world*. Bare `World()` is the legacy two-factor `REGISTRY`; the
**scored** inventory-management task is the three-factor `CORE`
(disruption + supplier + demand); `RICH` is the full six-factor world.

```python
from src.world import World, WorldConfig
from src.world.registry import CORE, RICH

w = World()                     # legacy 2-factor world
w = World(WorldConfig(), CORE)  # scored 3-factor inventory task
w = World(WorldConfig(), RICH)  # full 6-factor world
obs = w.reset(seed=1)
...
| `registry.py` | — | The `Module` record + the `REGISTRY` (legacy 2-factor), `CORE` (scored 3-factor), and `RICH` (6-factor) tuples. Composing the modules. |
...
2. Validate the action (`0 <= qty <= order_max`; route + a live-contracted
   supplier when `qty > 0`).
...
- **RNG exogeneity.** Kernels consume the rng in `registry` order; **actions
  ...
  alone* — reproducible across every policy and baseline run on that seed. New
  factors are *appended* in `RICH`, so their draws come last and the
  disruption/supplier trajectories are unperturbed.
...
`test_world.py` (~126 fast tests) pins the factored dynamics, the cost
arithmetic, and the registry draw order, so any change that perturbs them fails
the suite. Because new factors are *appended* in `CORE`/`RICH`, the legacy
two-factor base trajectory stays unperturbed when modules are added.
