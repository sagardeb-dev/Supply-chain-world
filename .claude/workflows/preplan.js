...
const DIMENSIONS = [
  { key: 'docs', targets:
    `Docs & memory. Do NOT work from a fixed file list -- DISCOVER every living doc: glob ${ROOT}/backend/**/*.md (this already covers every AGENTS.md and README.md), EXCLUDING .venv/, .pytest_cache/, and docs/superpowers/specs/ (point-in-time design specs, not living docs). Read every match -- especially the easy-to-miss ones that a hardcoded list drops: src/api/README.md, src/world/modules/README.md, src/world/substrate/README.md, and the whole claude-mds/ tree (architecture.md, dependency-graph.md), which tend to be the MOST stale. ALSO read every *.md under /home/azureuser/.claude/projects/-data-supply-chain-pomdp/memory/. Flag every CLAIM that is now false or misleading under the new direction (e.g. "masked-distress supplier task IS the benchmark", "CausalOracle is 2-factor only / its value is pinned", "place_order is the only week-advancing tool" if reframed, anything implying flat demand or minimal buffers, order qty fixed to {0,20,40}). Put findings in stale_docs with the exact file + section/line + the claim + why it is stale + the concrete fix.` },
  { key: 'scripts', targets:
